...
\end{align*}
O sea que los meridianos sí son geodésicas.

Ahora veamos cuáles paralelos son geodésicas tomando $u=u(s)$ y $v=$ constante. La primera ecuación se traduce en que $u'=$ constante y la segunda se vuelve
\[\frac{ff'}{(f')^2+(g')^2}(u')^2=0\]
Ahora $u'\neq0$ para que sí estemos hablando de una curva. Y además, como el denominador es precisamente el valor de $G$, no puede ser cero. Y por fin, $f$ tampoco puede ser cero para que no colapse la superficie. Así es que si nuestra curva es geodésica, $f'=0$, que son los paralelos cuyo radio alcanza un mínimo o un máximo. En otras palabras, se trata de los paralelos generados por puntos en la generatriz cuya recta tangente es paralela al eje z como en la figura.
\begin{figure}
	\centering
